...
# Example usage with (9, 35, 3) array of player poses
...
def calculate_features(player_id_frame,frame, match_data, action_name):
    ...
    # Calculating joint angles and rounding to 2 decimal places
    hip_angle_left_3D = round(calculate_angle_3d(j3d[5], j3d[4], j3d[0]), 2)
    knee_angle_left_3D = round(calculate_angle_3d(j3d[6], j3d[5], j3d[4]), 2)
    ankle_angle_left_3D = round(calculate_angle_3d(j3d[30], j3d[6], j3d[5]), 2)

    hip_angle_right_3D = round(calculate_angle_3d(j3d[2], j3d[1], j3d[0]), 2)
    knee_angle_right_3D = round(calculate_angle_3d(j3d[3], j3d[2], j3d[1]), 2)
    ankle_angle_right_3D = round(calculate_angle_3d(j3d[21], j3d[3], j3d[2]), 2)

    hip_angle_left_2D = round(calculate_angle_2d(j2d[5], j2d[4], j2d[0]), 2)
    knee_angle_left_2D = round(calculate_angle_2d(j2d[6], j2d[5], j2d[4]), 2)
    ankle_angle_left_2D = round(calculate_angle_2d(j2d[30], j2d[6], j2d[5]), 2)

    hip_angle_right_2D = round(calculate_angle_2d(j2d[2], j2d[1], j2d[0]), 2)
    knee_angle_right_2D = round(calculate_angle_2d(j2d[3], j2d[2], j2d[1]), 2)
    ankle_angle_right_2D = round(calculate_angle_2d(j2d[21], j2d[3], j2d[2]), 2)
    ...
    # linear_position_2D = {
    #     "pelvis": np.round(j2d_norm_human[0], 2)
    # }

    # linear_velocity_2D = {
    #     "pelvis": np.round(np.array([0, 0]), 2)
    # }
    ...
    linear_position_3D = {
        "right_leg": {
            "ankle": np.round(j3d_human[3], 2),
            "knee": np.round(j3d_human[2], 2),
        },
        "left_leg": {
            "ankle": np.round(j3d_human[6], 2),
            "knee": np.round(j3d_human[5], 2),
        }
    }
...
def process_clip_file(clip_file_path, match_data, action_name):
    ...
    with open(clip_file_path, 'r') as file:
        for i, line in enumerate(file):
            ...
            #player_data["linear_position_2D"] = l_pos_2D
            #player_data["linear_velocity_2D"] = l_vel_2D
            #player_data["linear_acceleration_2D"] = l_acc_2D
...
def compute_velocities_and_accelerations(frames, frame_rate, frame_gap=5):
    ...
    for leg in ['right_leg', 'left_leg']:
        for joint in ['hip', 'knee', 'ankle']:
            ...
            default_value = round(float(pi), 2) if joint == 'ankle' else round(float(2 * pi), 2) if joint == 'knee' else round(float(pi), 2)
            corrected_angle_2D = replace_nan_with_last_valid_or_default(
                curr_angle_2D,
                default_value=default_value,
                prev_valid_value=default_value
            )
    ...
    for i in range(1, len(frames)):
        ...
        for leg in ['right_leg', 'left_leg']:
            ...
            for joint in ['hip', 'knee', 'ankle']:
                ...
                angular_velocity_3D[joint] = (curr_angle_3D - prev_angle_3D) / dt
