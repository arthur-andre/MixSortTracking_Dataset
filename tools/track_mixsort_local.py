...
def make_parser():
    ...
    parser.add_argument("-ofn", "--output_folder_name", type=str, default=None, help="output folder name")
    # distributed
    parser.add_argument(
    ...
    parser.add_argument("--nba_videos",type=bool,default=False)
    parser.add_argument("--nba_match",type=str,default='CHI_NYK')
    return parser
...
def main_bis(exp, args, num_gpu):
    ...
    file_name = os.path.join(exp.output_dir, args.nba_match)
...
#######  ITERATION  ########
    ...
    json_actions_path = directory_path+'/sorted_actions.json'
    with open(json_actions_path, 'r') as f:
...
def main(exp, args, num_gpu):
    if args.nba_videos:
        ...
        val_loader = exp.get_eval_loader(args.batch_size, is_distributed, args.test, return_origin_img=True, output_folder_name = args.output_folder_name)
        evaluator = MOTEvaluator(
            args=args,
            dataloader=val_loader,
            img_size=exp.test_size,
