...
def kalman_filter_3d(joints_3D):
    """
    Apply a Kalman filter to smooth the 3D joint data.
    
    Args:
    joints_3D: (N, 35, 3) array of joint positions over N frames.
    
    Returns:
    smoothed_poses: Smoothed 3D joint data.
    """

    flat_poses = joints_3D.reshape(joints_3D.shape[0], -1)

    # Initialize Kalman filter
    kf = KalmanFilter(initial_state_mean=flat_poses[0], n_dim_obs=flat_poses.shape[1])

    # Apply Kalman filter to smooth the data
    smoothed_states, _ = kf.smooth(flat_poses)

    # Reshape the smoothed data back to original format (frames, joints, 3D)
    smoothed_poses = smoothed_states.reshape(joints_3D.shape)

    return smoothed_poses

def apply_kalman_filter_to_pose_data(frames, max_frame_gap=5):
    """
    ...
    for segment in all_segments:
        if len(segment) == 1:
            ...
            frames[incr]['j3d_human']['coordinates'] = segment[0]['j3d_human']['coordinates']
            incr += 1
            continue

        # Extract the pose data for the current segment
        segment_poses = [frame['j3d_human']['coordinates'] for frame in segment]
        ...
        # Apply the Kalman filter to smooth the segment
        smoothed_poses = kalman_filter_3d(pose_data_np)
        #smoothed_poses = pose_data_np
        # Update the original pose data with the smoothed results
        for i, frame in enumerate(segment):
            frames[incr]['j3d_human']['coordinates'] = np.round(smoothed_poses[i],3)
            incr += 1
...
def process_all_players(data, frame_rate, frame_gap=5):
    ...
    for player_id, player_info in data['players'].items():
        ...
        frames = apply_kalman_filter_to_pose_data(frames)
